...
class HistoricCSVDataHandlerHFT(DataHandler):
  """
  HistoricCSVDataHandlerHFT is designed to read CSV files for each requested symbol from disk and provide an interface to obtain the "latest" bar in a manner identical to a live trading interface.
  """
  ...
  def _open_convert_csv_files(self): 
    ...
    for s in self.symbol_list:
      ...
      self.symbol_data[s] = pd.io.parsers.read_csv(os.path.join(self.csv_dir, ’%s.csv’ % s), header=0, index_col=0, parse_dates=True,
                                                 names=[’datetime’, ’open’, ’high’,’low’, ’close’, ’volume’, ’oi’]).sort()
